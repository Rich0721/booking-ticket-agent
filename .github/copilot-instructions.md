...
由PM提出新需求，開發完成僅會填寫完成開發欄位，請依照以下格式進行回覆:

```Markdown
### Requirement - [需求簡述]
- 分支名稱: [請提供新需求或bug修復的分支名稱]
- 需求說明: [請描述新需求或bug修復]
...
由PM提出Bug修復需求，此步驟有兩種情境，分別為**Bug確認**與**Bug修復**:

- Bug確認: PM會提出確認Bug來源，並回填**錯誤原因**，不能調整程式碼，待PM確認後，才可以進行Bug修復
- Bug修復: PM已確認Bug來源，會直接要求進行Bug修復，可調整程式碼

```Markdown
### Bug - [Bug簡述]
- 分支名稱: [請提供Bug修復的分支名稱]
- 需求說明: [請描述Bug情境與修復需求]
- 錯誤原因: [PM要求確認Bug來源時，由Agent確認後回填，待PM確認後再進行修復]
- 完成開發:
- PM確認:
- 完成開發: [空白代表未完成開發，若完成開發請填寫完成開發日期]
- PM確認: [由PM進行填寫]
...
- 先檢查分支名稱有沒有存在地端
  - 有，則切換過去後進行遠端pull最新的版本
  - 沒有，則先切換到main分支，pull最新的版本後，開出對應功能分支
- 分支命名規則，根據溝通文件進行分支命名，若溝通文件沒有定義則直接回覆PM，請PM提供分支命名規則，而不進行任何開發或修復行為
...
- 除了指定Markdown文件外，請勿生成其他Markdown文件，並且不要生成任何圖片或圖表
